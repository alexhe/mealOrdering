...
<div class="pMenu" id="id_pMenu">
	...
	<li class="fdpic" onclick="master_open({url:'?app=user.config&app_module=sys&key=sys.user.repayment',title:'设置字段',w:400});">&nbsp;</li>
</div>
...
admin.table.list1.save_resize = function() {
	...
	kj.ajax.get("common.php?app=config&app_module=user&dir=<?php echo $app_dir;?>&app_act=save_resize&key=sys.user.repayment&index=" + this.fieldsindex + "&w=" + lng_w , function(data){
		//alert(data);
	});
}
admin.table.list1.sort = function(key) {
	kj.ajax.get("common.php?app=config&app_module=user&dir=<?php echo $app_dir;?>&app_act=sort&key=sys.user.repayment&sortby=" + key , function(data){
