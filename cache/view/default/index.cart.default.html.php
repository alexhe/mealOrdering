...
<div class="main">
	...
	<div class="div_3" id="id_list_0">
		<div class="x_tit1"><li class="x_col1">序号</li><li class="x_col2 main_w8">所选菜品</li><li class="x_col3">单价</li><li class="x_col3">数量</li><li class="x_col3">金额小计</li><li class="x_col4">操作</li></div>
		<?php foreach($cart_list['cart'] as $key=>$cart){ ?>
		<div class="x_list1"><div class="x_col1"><?php echo $cart['index'];?></div>
		<div class="x_col2 main_w8">
			<?php foreach($cart['ids'] as $menu){ ?>
			<?php echo $cart_list['menu']['id_'.$menu]['menu_title'];?>
			<?php }?>
		</div><div class="x_col3"><?php echo $cart_list['menu']['id_'.$menu]['menu_price'];?></div><div class="x_col3"><?php echo $cart['num'];?></div><div class="x_col3 x_sel1"><span class="menu_price">￥<?php echo $cart['price'];?></span></div><div class="x_col4"><span onclick="thisjs.cart_remove(this , 0,<?php echo $menu;?>)" style="cursor:pointer" class="x_sel2">【删除此列】</span></div></div>
		<?php }?>
	</div>
	<script>
	kj.onload(function(){
		thisjs.shopid = 0;
		thisjs.ordernum = <?php if(empty($cart_list['num'])){?>0<?php } else { ?><?php echo $cart_list['num'];?><?php }?>;
	...
	<div class="div_4" style="height:auto" id = 'x_act_div'>
	...
	<div class="div_4">
		...
		<?php }?>
			...
			<table class="x_box1<?php if(count($this_info['list'])<1){?> x_sel1<?php }?>" <?php if($this_info['isover']){?> style='display:none'<?php }?>>
				...
				<tr><td>所在区域：<span style="color:#ff0000">*</span></td><td>
					...
					<?php for($i=1;$i<$areainfo["depth"];$i++){ ?>
						<select name="info_area_id[]" onchange="thisjs.changearea(this.value,<?php echo $i;?>);" id="id_area_<?php echo $i;?>">
						</select>
					<?php }?>					
			...
			<table class="x_box1 x_sel1" id="id_edit_infodiv" style="display:none">
				...
				<td valign="top"><a href="javascript:thisjs.info_save();"><img src="/mealOrdering/webcss/default/images/btn_submit2.gif"></a>&nbsp;&nbsp;<a href="javascript:thisjs.info_cancel(true);">取消</a>			</td></tr>
			</table>
		</div>
	</div>
	<div class="div_4" id="id_score_0">
		...
		<span><?php echo $arrivetime['arriveday'];?> <?php echo $arrivetime['arrivetime'];?></span>
		<input name="arrive" type="hidden" value="<?php echo $arrivetime['arriveinput'];?>"/>
		<!-- 
		<select name="arrive">
	...
	<div class="div_5">
		...
		<?php } else { ?>
		<a href="javascript:thisjs.submit(0);"><img src="/mealOrdering/webcss/default/images/btn_submit2.gif"></a>
		<?php }?>
	</div>
	<?php }?>
</div>
...
var thisjs = new function() {
	...
	this.is_alert = false;
	this.shopid = 0;
	this.score = kj.toint('<?php echo $score_total;?>');//当前积分
	...
	this.arriveday =  <?php echo $arrivetime['arrivedaykey'];?> ;
		...
		if(this.act_list.length>0) {
			...
			for(var i = 0; i < this.act_list.length ; i++) {
				...
				if(this.act_list[i]['act_where'] == '1') {//达到指定金额
					...
					if(this.act_list[i]['where_val']<=this.ordernum) {
						if(!obj_act) {
						    obj_act=document.createElement("li");
							obj_act.id="id_act_"+this.shopid+"_"+this.act_list[i]['act_id'];
							obj_act.innerHTML=this.act_list[i]['act_name'] + "<input type='hidden' name='act_money[]' value='"+price+"' id='id_act_money_"+this.act_list[i]['act_id']+"'><input type='hidden' name='shop_act_id[]' value='"+this.act_list[i]['act_id']+"'>";
				...
				} else if(this.act_list[i]['act_where']== '5'){
					if(this.act_list[i]['where_val'] == this.arriveday) {
	                        if(!obj_act) {
	                            obj_act=document.createElement("li");
	                            obj_act.id="id_act_"+this.shopid+"_"+this.act_list[i]['act_id'];
	                            obj_act.innerHTML = this.act_list[i]['act_name'] + "<input type='hidden' name='act_money[]' value='"+price+"' id='id_act_money_"+this.act_list[i]['act_id']+"'><input type='hidden' name='shop_act_id[]' value='"+this.act_list[i]['act_id']+"'>";
	                        ...
	                        if(obj_act) kj.remove(obj_act);//移除
	                        var act_div = kj.obj('#x_act_div').style.display="none";
	                }
				}
	...
	this.get_act_money = function(id , method , method_val, total_price) {
		...
		switch(method) {
			...
			case 5://立减多少
				...
				break;
			case 6://每份优惠多少
				money = kj.toint(method_val) * this.ordernum;
				break;
		}
		return money;
	}
	this.clear_act_time = function() {
		//超时，移除时间优惠
		kj.remove('#id_act_where_time');
		this.time_money = 0;
		this.refresh_price();
	}
	<?php if($this_login_user->uid<1){?>
